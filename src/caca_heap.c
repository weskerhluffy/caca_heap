...
typedef enum {
	anadir_caca_heap_tipo_consulta = 1,
	borrar_caca_heap_tipo_consulta,
	imprimir_caca_heap_tipo_consulta,
	invalida_caca_heap_tipo_consulta
} caca_heap_tipo_consulta;
static inline void caca_heap_main() {
	natural consultas_tam = 0;
	heap_shit *ctx = NULL;

	ctx = heap_shit_init(verdadero);

	scanf("%u\n", &consultas_tam);
	for (int i = 0; i < consultas_tam; i++) {
		caca_heap_tipo_consulta tipo = invalida_caca_heap_tipo_consulta;
		tipo_dato valor;
		scanf("%u", &tipo);
		switch (tipo) {
		case anadir_caca_heap_tipo_consulta:
			scanf("%d", &valor);
			caca_log_debug("anadiendo %d", valor)
			;
			heap_shit_insertar_valor_unico(ctx, valor);
			break;
		case borrar_caca_heap_tipo_consulta:
			scanf("%d", &valor);
			caca_log_debug("borrando %u", valor)
			;
